...
import { useAmenities, useDeleteAmenity } from "./hooks";
import { AmenityForm } from "./form";
import { AMENITY_STATUSES, type Amenity } from "./types";

const statusLabel = labelMap(AMENITY_STATUSES);

/** Amenities catalog list — search, status facet, create, per-row edit and delete. */
export function AmenitiesList() {
  ...
  const activeFilters: ActiveFilter[] = status
    ? [{ key: "status", label: `Status: ${statusLabel[status as Amenity["status"]]}` }]
    : [];
  ...
  const handleExport = () => {
    exportToCsv<Amenity>("amenities", list.rows, [
        ...
        filterControls={
          <Select
            ...
            options={[
              ...
              ...statusOptions(AMENITY_STATUSES),
            ]}
