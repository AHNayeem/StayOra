import type { ColumnDef } from "../../crud";
import { StatusBadge } from "../../ui";
import { formatCurrency, formatDate } from "../../lib/format";
import { labelMap, toneMap } from "../../lib/status";
import { COMMISSION_STATUSES, type Commission } from "./types";

const statusTone = toneMap(COMMISSION_STATUSES);
const statusLabel = labelMap(COMMISSION_STATUSES);

export const commissionColumns: ColumnDef<Commission>[] = [
  {
    accessorKey: "reference",
    header: "Reference",
    enableHiding: false,
    meta: { label: "Reference" },
    cell: ({ row }) => (
      <span className="font-medium text-ink">{row.original.reference}</span>
    ),
  },
  {
    accessorKey: "merchant",
    header: "Merchant",
    meta: { label: "Merchant" },
    cell: ({ row }) => (
      <div className="min-w-0">
        <p className="truncate text-ink">{row.original.merchant}</p>
        <p className="truncate text-xs text-muted">{row.original.bookingRef}</p>
      </div>
    ),
  },
  {
    accessorKey: "bookingAmount",
    header: "Booking",
    meta: { label: "Booking", align: "right" },
    cell: ({ row }) => (
      <span className="tabular-nums text-body">
        {formatCurrency(row.original.bookingAmount, row.original.currency)}
      </span>
    ),
  },
  {
    accessorKey: "rate",
    header: "Rate",
    meta: { label: "Rate", align: "right" },
    cell: ({ row }) => (
      <span className="tabular-nums text-body">{row.original.rate}%</span>
    ),
  },
  {
    accessorKey: "commissionAmount",
    header: "Commission",
    meta: { label: "Commission", align: "right" },
    cell: ({ row }) => (
      <span className="font-medium tabular-nums text-ink">
        {formatCurrency(row.original.commissionAmount, row.original.currency)}
      </span>
    ),
  },
  {
    accessorKey: "status",
    header: "Status",
    meta: { label: "Status" },
    cell: ({ row }) => (
      <StatusBadge tone={statusTone[row.original.status]}>
        {statusLabel[row.original.status]}
      </StatusBadge>
    ),
  },
  {
    accessorKey: "createdAt",
    header: "Date",
    meta: { label: "Date" },
    cell: ({ row }) => (
      <span className="whitespace-nowrap text-body">
        {formatDate(row.original.createdAt)}
      </span>
    ),
  },
];
